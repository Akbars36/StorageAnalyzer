...
class Draw {
	...
	_point2pixel(point) {
		var pY = point[1]?point[1]:0;
		return {
			x: this.canvas.width  * this.xRangeInv*(point[0]-this.xMin),
			y: this.canvas.height * this.yRangeInv*(pY-this.yMin)
		};		
	}
	...
	circle(mean, variance, color) {   // assuming cov matrix is symmetric
		if(!color) color = 'black';
		let w = this.canvas.width;
		let h = this.canvas.height;
		let r1 = Math.sqrt(variance)*4;
		...
		this.ctx.beginPath();
		this.ctx.arc(x, y, r1pix, 0, 2*Math.PI);
		//this.ctx.fill();
		this.ctx.stroke();
		this.ctx.globalAlpha = 1;
	}
