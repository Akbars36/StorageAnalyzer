...
kmeans = {
     ...
     */
    findClosestCentroid:function (centroids, observation) {
        const k = centroids.length; // number of clusters/centroids
 
        let centroid = 0;
        let minDistance = this.distance(centroids[0], observation);
        for(let i = 1; i < k; i += 1) {
            const dist = this.distance(centroids[i], observation);
            if(dist < minDistance) {
                centroid = i;
                minDistance = dist;
            }
     ...
     */
    calculateCentroid:function (observations) {
     ...
     */
    assignClusters:function(centroids, observations) {
        const n = observations.length;  // number of observations
     ...
     */
    kmeansStep:function (centroids, observations) {
        const k = centroids.length; // number of clusters/centroids
        ...
        for(let i = 0; i < k; i += 1) {
            ...
            newCentroids.push(this.calculateCentroid(clusteredObservations));
        }
     ...
     */
    kmeans:function (model, maximumIterations = 200, converged = assignmentsConverged) {
     ...
     */
    centroidsConverged:function (delta) {
        /**
     ...
     * @param number delta - the maximum difference between each centroid in consecutive runs for convergence
     */
    assignmentsConverged:function (model, newModel) {
     ...
     */
    assignmentsToClusters:function (model) {
        // 
