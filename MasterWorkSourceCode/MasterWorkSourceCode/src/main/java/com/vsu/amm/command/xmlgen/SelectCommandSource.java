...
public class SelectCommandSource implements ICommandSource {
	...
	public SelectCommandSource(Element elem, AliasSet aliasSet,
		...
		if (reuseValues && valueSet.hasStoredValues())
			...
			for (int i = 0; i < count; i++)
				commands.add(new SelectCommand(valueSet
