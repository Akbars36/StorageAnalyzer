...
public class StorageGenerator {
    ...
    public static List<IDataStorage> generateStorages(Element element, Map<String, Integer> params) {
        if (element == null)
            return null;
        ...
        for (Element child : children) {
            ...
            for (Element storageParam : child.getChildren()) {
                if ("param".equals(storageParam.getName())) {
                    ...
                    storageParams.put(storageParamName, Integer.parseInt(storageParamValue));
                    ...
                    String cacheInsertRate = storageParam.getAttributeValue("rate");
                    cache = getCache(cacheClass, cacheSize, cacheInsertRate, params);
                }
            }
    ...
    private static AbstractCache getCache(String cacheClass, String cacheSize, String cacheInsertRate, Map<String, Integer> params) {
        if (cacheClass == null)
            return null;
        try {
            Constructor<?> c = Class.forName(cacheClass).getConstructor(String.class, String.class, Map.class);
            return (AbstractCache) (c.newInstance(cacheSize, cacheInsertRate, params));
    ...
    public static IDataStorage getDataStorage(String storageName, AbstractCache cache,
                                              Map<String, Integer> storageParams) {
        IDataStorage result;
        if (storageName == null)
            return null;
