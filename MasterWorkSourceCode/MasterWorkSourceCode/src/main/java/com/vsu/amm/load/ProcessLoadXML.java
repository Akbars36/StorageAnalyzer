...
public class ProcessLoadXML {
            ...
            for(Pair<String, String> pair  : result){
                switch (pair.getKey()){
                    case Constants.INSERT_COMMAND_NAME:
                        storage.set(Integer.parseInt(pair.getValue()));
                        break;
    ...
    public void processXMLNew(String filename, IDataStream stream){
        ...
        try {
            ...
            for(Element storageNode : (List<Element>)storagesNode.getChildren()){
            ...
            for(Element param : (List<Element>)paramsNode.getChildren()){
                param_name = param.getAttributeValue("name");
                param_values = new ArrayList<>();
                for(Element value : (List<Element>)param.getChildren()){
